import { MatchResult, Match } from './matcher';
...
export interface JsonReport {
  ...
  results: Array<{
    ...
    matches: Array<{
      ...
      line: number;
      column: number;
      maskedPreview: string; // NEVER the raw value
    }>;
  }>;
}
 ...
 */
export function formatMatchLine(match: Match, source: string): string {
...
export function formatTextReport(summary: ScanSummary, opts: ReportOptions): string {
  const lines: string[] = [
    'ai-output-redacter',
    '===================',
    'Mode: ' + opts.mode,
...
export function formatMarkdownReport(summary: ScanSummary, opts: ReportOptions): string {
  const lines: string[] = [];
...
export function buildJsonReport(summary: ScanSummary, opts: ReportOptions): JsonReport {
