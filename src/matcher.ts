...
export interface Match {
  ...
  column: number;
  length: number;
  maskedPreview: string; // first 6 chars + *** + [N chars] — NEVER the full value
}
...
export function matchPattern(text: string, pattern: Pattern): Match[] {
  const matches: Match[] = [];
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (pattern.luhnCheck && !luhnCheck(value)) continue;
...
export function matchAll(text: string, patterns: Pattern[]): Match[] {
  const all: Match[] = [];
...
export function matchText(text: string, patterns: Pattern[], source = 'stdin'): MatchResult {
